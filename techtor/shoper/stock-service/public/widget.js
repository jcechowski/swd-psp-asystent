...
`,Q="inpost-izi-button, INPOST-IZI-BUTTON { display: none !important; }",Z='product-ask-questions, [data-module-name="product_ask_questions"] { display: none !important; }';var g=class{constructor(){this.priceStyleEl=null}apply(t,e,r){let o=document.querySelectorAll('buy-button, [class*="buy-button"], .product-buy__button'),s=document.querySelector('h-input-stepper, [class*="quantity__input"]')?.closest('product-quantity, [class*="quantity"], .product-quantity'),d=t==="price-zero",p=t==="price-zero";for(let c of o){p?(c.classList.add("techtor-hide"),c.dataset.techtorHidden="1"):(c.classList.remove("techtor-hide"),delete c.dataset.techtorHidden);let l=c.querySelector('.btn_primary, button[type="submit"]')||(c.classList.contains("btn_primary")?c:null);l&&(d?(l.setAttribute("disabled","true"),l.style.opacity="0.5",l.style.pointerEvents="none"):(l.removeAttribute("disabled"),l.style.opacity="",l.style.pointerEvents="")),d?c.setAttribute("is-buyable","0"):c.removeAttribute("is-buyable")}s&&(p?(s.classList.add("techtor-hide"),s.dataset.techtorHidden="1"):(s.classList.remove("techtor-hide"),delete s.dataset.techtorHidden)),p?this.priceStyleEl||(this.priceStyleEl=document.createElement("style"),this.priceStyleEl.id="techtor-price-hide",this.priceStyleEl.textContent=`
  ...
  </div>`}var rt=["techtor-ask-banner","techtor-tarnawa-banner","techtor-price0-banner","techtor-overlimit-banner"],v=class{constructor(){this.onAskClick=null}setAskHandler(t){this.onAskClick=t}apply(t,e,r){this.removeAll();let o=document.querySelector('buy-button, .product-actions, [data-module-name="product_actions"], .product-buy')?.closest('.product-actions, [data-module-name="product_actions"]')||document.querySelector("buy-button")?.parentElement;if(!o)return;let i="";switch(t){case"available-tarnawa":i=J();break;case"out-of-stock":i=K();break;case"price-zero":i=V();break;case"overlimit":i=Y(e.totalStock);break;default:return}let s=document.createElement("div");s.innerHTML=i;let d=s.firstElementChild;o.insertBefore(d,o.firstChild);let p=d.querySelector('[id$="-ask"]');if(p&&this.onAskClick){let c=this.onAskClick,l=document.querySelector("h1")?.textContent?.trim()||"";p.addEventListener("click",()=>{c(e.sku,l,t==="overlimit"?r:void 0,t==="price-zero")})}}destroy(){this.removeAll()}removeAll(){for(let t of rt)document.getElementById(t)?.remove()}};var x=class{constructor(){this.styleEl=null;this.observer=null}apply(t,e,r){let o=document.querySelector(".product-availability dd, .product-availability span, .product-availability .property__value");if(!o)return;let i="",s="";switch(t){case"available":i="dost\u0119pny",s="#16a34a";break;case"available-tarnawa":i="dost\u0119pny",s="#16a34a";break;case"overlimit":i="zapytaj o dost\u0119pno\u015B\u0107",s="#b45309";break;case"out-of-stock":i="zapytaj o dost\u0119pno\u015B\u0107",s="#b45309";break;case"price-zero":i="zapytaj o cen\u0119 i dost\u0119pno\u015B\u0107",s="#b45309";break}if(!i)return;let d="techtor-avail-css";this.styleEl||(this.styleEl=document.createElement("style"),this.styleEl.id=d,document.head.appendChild(this.styleEl)),this.styleEl.textContent=`
    ...
    <div style="background:#fff;max-width:780px;width:100%;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,0.15);max-height:90vh;overflow-y:auto;padding:28px;position:relative;">
      ...
      <p style="margin:0 0 20px;font-size:13px;color:#6b7280;">Produkt: <strong>${m(t)}</strong> (${m(n)})</p>
  ...
  `,document.body.appendChild(o),document.getElementById("techtor-ask-close").onclick=()=>o.remove();let d=o.querySelector('input[name="nip"]');d&&d.addEventListener("input",async()=>{let c=d.value.replace(/\D/g,"");if(c.length===10)try{let b=await(await fetch(`${st}?nip=${c}`)).json();if(b?.result?.subject){let u=b.result.subject,M=o.querySelector("form"),_=(q,y)=>{let T=M.querySelector(`[name="${q}"]`);T&&!T.value&&(T.value=y)};_("company",u.name||""),_("street",u.workingAddress?.split(",")[0]||"");let I=(u.workingAddress||"").split(",");if(I.length>1){let y=I[I.length-1].trim().match(/^(\d{2}-\d{3})\s+(.+)/);y&&(_("zip",y[1]),_("city",y[2]))}a("NIP auto-fill:",u.name)}}catch{}});let p=document.getElementById("techtor-ask-form");p.onsubmit=async c=>{c.preventDefault();let l=new FormData(p),b=(l.get("name")||"").trim(),u=(l.get("email")||"").trim();if(!b||!u||!u.includes("@")){alert("Wype\u0142nij wymagane pola (imi\u0119, email)");return}try{(await fetch(it,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:b,email:u,_hp:l.get("_hp")||"",phone:l.get("phone")||"",nip:l.get("nip")||"",company:l.get("company")||"",street:l.get("street")||"",zip:l.get("zip")||"",city:l.get("city")||"",message:l.get("message")||"",quantity:l.get("quantity")||"1",sku:n,product:t,url:location.href})})).ok?(p.style.display="none",document.getElementById("techtor-ask-success").style.display="block",setTimeout(()=>o.remove(),3e3)):alert("Wyst\u0105pi\u0142 b\u0142\u0105d. Spr\xF3buj ponownie.")}catch{alert("B\u0142\u0105d po\u0142\u0105czenia. Spr\xF3buj ponownie.")}}}var S=class{constructor(t){this.modules=[];this.currentState="loading";this.currentQty=1;this.currentSku="";this.adapter=t}start(){this.injectStyles();let t=new v;t.setAskHandler(W),this.modules=[new g,t,new x,new k,new w],this.adapter.onProductChange(r=>{a("SKU changed:",r),this.currentSku=r,this.update()}),this.adapter.onQuantityChange(r=>{a("Qty changed:",r),this.currentQty=r,this.update()}),this.adapter.onNavigation(()=>{a("Navigation detected \u2014 restart"),this.restart()});let e=h();if(e)this.currentSku=e,this.update();else{let r=setInterval(()=>{let o=h();o&&(clearInterval(r),this.currentSku=o,this.update())},200);setTimeout(()=>clearInterval(r),1e4)}window._tRerun=()=>this.restart(),a("Widget started, mode:",window.__techtorWidget?.mode)}update(){if(!this.currentSku)return;let t=P(this.currentSku),e=D(t,this.currentQty);N(this.currentState,e,t,this.currentQty),this.currentState=e,window.__techtorWidget&&(window.__techtorWidget.state=e);for(let r of this.modules)r.apply(e,t,this.currentQty)}restart(){a("Widget restart");for(let t of this.modules)t.destroy();$(),this.currentState="loading",this.currentQty=1,this.currentSku="",H().then(()=>{let t=new v;t.setAskHandler(W),this.modules=[new g,t,new x,new k,new w];let e=h();e&&(this.currentSku=e,this.update())})}injectStyles(){if(!document.getElementById("techtor-widget-css")){let t=document.createElement("style");t.id="techtor-widget-css",t.textContent=R+`
`+Z,document.head.appendChild(t)}}};async function G(){if(!window.__techtorWidget?.initialized)if(window.__techtorWidget={version:3,runId:0,initialized:!1,mode:"loading"},O(),a("Widget v3 bootstrap start"),await H(),typeof window.useStorefront=="function")window.useStorefront(n=>{if(window.__techtorWidget.initialized)return;window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="eventbus",a("Tryb: Event Bus (useStorefront)");let t=new E(n);new S(t).start()}),setTimeout(()=>{if(!window.__techtorWidget.initialized){A("useStorefront timeout \u2014 fallback na DOM adapter"),window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="dom-fallback";let n=new f;new S(n).start()}},5e3);else{A("Brak useStorefront \u2014 DOM fallback"),window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="dom-fallback";let n=new f;new S(n).start()}}G().catch(n=>console.error("[TECHTOR] Widget bootstrap error:",n));})();

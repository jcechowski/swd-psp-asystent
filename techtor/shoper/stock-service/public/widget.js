...
`,F="inpost-izi-button, INPOST-IZI-BUTTON { display: none !important; }",Z='product-ask-questions, [data-module-name="product_ask_questions"] { display: none !important; }';var w=class{constructor(){this.priceStyleEl=null}apply(t,e,r){let o=document.querySelectorAll('buy-button, [class*="buy-button"], .product-buy__button'),s=document.querySelector('h-input-stepper, [class*="quantity__input"]')?.closest('product-quantity, [class*="quantity"], .product-quantity'),d=t==="price-zero",u=t==="price-zero";for(let p of o){u?(p.classList.add("techtor-hide"),p.dataset.techtorHidden="1"):(p.classList.remove("techtor-hide"),delete p.dataset.techtorHidden);let l=p.querySelector('.btn_primary, button[type="submit"]')||(p.classList.contains("btn_primary")?p:null);l&&(d?(l.setAttribute("disabled","true"),l.style.opacity="0.5",l.style.pointerEvents="none"):(l.removeAttribute("disabled"),l.style.opacity="",l.style.pointerEvents="")),d?p.setAttribute("is-buyable","0"):p.setAttribute("is-buyable","1")}s&&(u?(s.classList.add("techtor-hide"),s.dataset.techtorHidden="1"):(s.classList.remove("techtor-hide"),delete s.dataset.techtorHidden)),u?this.priceStyleEl||(this.priceStyleEl=document.createElement("style"),this.priceStyleEl.id="techtor-price-hide",this.priceStyleEl.textContent=`
  ...
  </div>`}var it=["techtor-ask-banner","techtor-tarnawa-banner","techtor-price0-banner","techtor-overlimit-banner"],S=class{constructor(){this.onAskClick=null}setAskHandler(t){this.onAskClick=t}apply(t,e,r){this.removeAll();let o=document.querySelector('buy-button, .product-actions, [data-module-name="product_actions"], .product-buy')?.closest('.product-actions, [data-module-name="product_actions"]')||document.querySelector("buy-button")?.parentElement;if(!o)return;let i="";switch(t){case"available-tarnawa":return;case"out-of-stock":i=V();break;case"price-zero":i=J();break;case"overlimit":i=Y(e.totalStock);break;default:return}let s=document.createElement("div");s.innerHTML=i;let d=s.firstElementChild;o.insertBefore(d,o.firstChild);let u=d.querySelector('[id$="-ask"]');if(u&&this.onAskClick){let p=this.onAskClick,l=document.querySelector("h1")?.textContent?.trim()||"";u.addEventListener("click",()=>{p(e.sku,l,t==="overlimit"?r:void 0,t==="price-zero")})}}destroy(){this.removeAll()}removeAll(){for(let t of it)document.getElementById(t)?.remove()}};var E=class{constructor(){this.styleEl=null;this.observer=null}apply(t,e,r){let o=document.querySelector(".product-availability dd, .product-availability span, .product-availability .property__value");if(!o)return;let i="",s="";switch(t){case"available":i="dost\u0119pny",s="#16a34a";break;case"available-tarnawa":i="dost\u0119pny",s="#16a34a";break;case"overlimit":i="zapytaj o dost\u0119pno\u015B\u0107",s="#b45309";break;case"out-of-stock":i="zapytaj o dost\u0119pno\u015B\u0107",s="#b45309";break;case"price-zero":i="zapytaj o cen\u0119 i dost\u0119pno\u015B\u0107",s="#b45309";break}if(!i)return;let d="techtor-avail-css";this.styleEl||(this.styleEl=document.createElement("style"),this.styleEl.id=d,document.head.appendChild(this.styleEl)),this.styleEl.textContent=`
  ...
  `,document.body.appendChild(o),document.getElementById("techtor-ask-close").onclick=()=>o.remove();let d=o.querySelector('input[name="nip"]');d&&d.addEventListener("input",async()=>{let p=d.value.replace(/\D/g,"");if(p.length===10)try{let b=await(await fetch(`${lt}?nip=${p}`)).json(),c=b?.data||b?.result?.subject;if(c){let q=o.querySelector("form"),h=(v,O)=>{let y=q.querySelector(`[name="${v}"]`);y&&!y.value&&(y.value=O)};if(h("company",c.name||""),h("street",c.street||c.workingAddress?.split(",")[0]||""),c.postalCode&&h("zip",c.postalCode),c.city&&h("city",c.city),c.email&&h("email",c.email),c.phone&&h("phone",c.phone),!c.postalCode&&c.workingAddress){let v=c.workingAddress.split(",");if(v.length>1){let y=v[v.length-1].trim().match(/^(\d{2}-\d{3})\s+(.+)/);y&&(h("zip",y[1]),h("city",y[2]))}}a("NIP auto-fill:",c.name)}}catch{}});let u=document.getElementById("techtor-ask-form");u.onsubmit=async p=>{p.preventDefault();let l=new FormData(u),b=(l.get("name")||"").trim(),c=(l.get("email")||"").trim();if(!b||!c||!c.includes("@")){alert("Wype\u0142nij wymagane pola (imi\u0119, email)");return}try{(await fetch(at,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:b,email:c,_hp:l.get("_hp")||"",phone:l.get("phone")||"",nip:l.get("nip")||"",company:l.get("company")||"",street:l.get("street")||"",zip:l.get("zip")||"",city:l.get("city")||"",message:l.get("message")||"",quantity:l.get("quantity")||"1",sku:n,product:t,url:location.href})})).ok?(u.style.display="none",document.getElementById("techtor-ask-success").style.display="block",setTimeout(()=>o.remove(),3e3)):alert("Wyst\u0105pi\u0142 b\u0142\u0105d. Spr\xF3buj ponownie.")}catch{alert("B\u0142\u0105d po\u0142\u0105czenia. Spr\xF3buj ponownie.")}}}function X(){let n=new S;return n.setAskHandler(G),[new w,n,new E,new _,new z,new I]}function tt(){let n=document.querySelector('h-input-stepper, [class*="quantity__input"]');if(!n)return 1;let t=n.querySelector('input[type="number"], input');if(t){let r=parseInt(t.value,10);if(r>0)return r}let e=parseInt(n.getAttribute("value")||"",10);return e>0?e:1}var L=class{constructor(t){this.modules=[];this.currentState="loading";this.currentQty=1;this.currentSku="";this.adapter=t}start(){this.injectStyles(),this.modules=X(),this.adapter.onProductChange(e=>{a("SKU changed:",e),this.currentSku=e,this.update()}),this.adapter.onQuantityChange(e=>{a("Qty changed:",e),this.currentQty=e,this.update()}),this.adapter.onNavigation(()=>{a("Navigation detected \u2014 restart"),this.restart()}),this.currentQty=tt(),a("Initial qty:",this.currentQty);let t=m();if(t)this.currentSku=t,this.update();else{let e=setInterval(()=>{let r=m();r&&(clearInterval(e),this.currentSku=r,this.update())},200);setTimeout(()=>clearInterval(e),1e4)}window._tRerun=()=>this.restart(),a("Widget started, mode:",window.__techtorWidget?.mode)}update(){if(!this.currentSku)return;let t=D(this.currentSku),e=R(t,this.currentQty);N(this.currentState,e,t,this.currentQty),this.currentState=e,window.__techtorWidget&&(window.__techtorWidget.state=e);for(let r of this.modules)r.apply(e,t,this.currentQty)}restart(){a("Widget restart");for(let t of this.modules)t.destroy();H(),this.currentState="loading",this.currentQty=1,this.currentSku="",C().then(()=>{this.modules=X(),this.currentQty=tt();let t=m();t&&(this.currentSku=t,this.update())})}injectStyles(){if(!document.getElementById("techtor-widget-css")){let t=document.createElement("style");t.id="techtor-widget-css",t.textContent=Q+`
`+Z,document.head.appendChild(t)}}},A=null;async function et(){if(window.__techtorWidget?.initialized&&A){a("Widget already initialized \u2014 rerun"),A.restart();return}window.__techtorWidget={version:3,runId:0,initialized:!1,mode:"loading"},P(),a("Widget v3 bootstrap start"),H(),await C();function n(t){let e=new L(t);A=e,e.start()}typeof window.useStorefront=="function"?(window.useStorefront(t=>{window.__techtorWidget.initialized||(window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="eventbus",a("Tryb: Event Bus (useStorefront)"),n(new x(t)))}),setTimeout(()=>{window.__techtorWidget.initialized||(T("useStorefront timeout \u2014 fallback na DOM adapter"),window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="dom-fallback",n(new g))},5e3)):(T("Brak useStorefront \u2014 DOM fallback"),window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="dom-fallback",n(new g))}et().catch(n=>console.error("[TECHTOR] Widget bootstrap error:",n));})();

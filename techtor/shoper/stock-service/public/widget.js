...
`,Q="inpost-izi-button, INPOST-IZI-BUTTON { display: none !important; }",Z='product-ask-questions, [data-module-name="product_ask_questions"] { display: none !important; }';var g=class{apply(t,e,n){let o=document.querySelectorAll('buy-button, [class*="buy-button"], .product-buy__button'),s=document.querySelector('h-input-stepper, [class*="quantity__input"]')?.closest('product-quantity, [class*="quantity"], .product-quantity'),d=t==="price-zero"||t==="overlimit",p=t==="price-zero";for(let c of o){p?(c.classList.add("techtor-hide"),c.dataset.techtorHidden="1"):(c.classList.remove("techtor-hide"),delete c.dataset.techtorHidden);let l=c.querySelector('.btn_primary, button[type="submit"]')||(c.classList.contains("btn_primary")?c:null);l&&(d&&!p?(l.setAttribute("disabled","true"),l.style.opacity="0.5",l.style.pointerEvents="none"):d||(l.removeAttribute("disabled"),l.style.opacity="",l.style.pointerEvents="")),d?c.setAttribute("is-buyable","0"):c.removeAttribute("is-buyable")}s&&(p?(s.classList.add("techtor-hide"),s.dataset.techtorHidden="1"):(s.classList.remove("techtor-hide"),delete s.dataset.techtorHidden))}destroy(){document.querySelectorAll("[data-techtor-hidden]").forEach(t=>{t.classList.remove("techtor-hide"),delete t.dataset.techtorHidden}),document.querySelectorAll("buy-button").forEach(t=>{t.removeAttribute("is-buyable");let e=t.querySelector(".btn_primary, button");e&&(e.removeAttribute("disabled"),e.style.opacity="",e.style.pointerEvents="")})}};var F='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#d97706" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',tt='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#dc2626" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>',U='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>';function K(){return`<div class="techtor-banner techtor-banner--warning" id="techtor-ask-banner">
    ...
    <div class="techtor-banner__row">${tt}<span class="techtor-banner__text techtor-banner__text--red">Zapytaj o aktualn\u0105 cen\u0119 i dost\u0119pno\u015B\u0107</span></div>
    <button class="techtor-ask-btn techtor-ask-btn--red" id="techtor-price0-ask">${U} Zapytaj o produkt</button>
  </div>`}function V(r){return`<div class="techtor-banner techtor-banner--warning" id="techtor-overlimit-banner">
    <div class="techtor-banner__row">${F}<span class="techtor-banner__text techtor-banner__text--amber">Brak wystarczaj\u0105cej ilo\u015Bci towaru</span></div>
    <p class="techtor-banner__detail">W magazynie posiadamy <strong>${r} szt.</strong> Zapytaj o dost\u0119pno\u015B\u0107 wi\u0119kszej ilo\u015Bci.</p>
    <button class="techtor-ask-btn techtor-ask-btn--amber" id="techtor-overlimit-ask">${U} Zapytaj o dost\u0119pno\u015B\u0107</button>
  </div>`}var et=["techtor-ask-banner","techtor-price0-banner","techtor-overlimit-banner"],v=class{constructor(){this.onAskClick=null}setAskHandler(t){this.onAskClick=t}apply(t,e,n){this.removeAll();let o=document.querySelector('buy-button, .product-actions, [data-module-name="product_actions"], .product-buy')?.closest('.product-actions, [data-module-name="product_actions"]')||document.querySelector("buy-button")?.parentElement;if(!o)return;let i="";switch(t){case"out-of-stock":i=K();break;case"price-zero":i=J();break;case"overlimit":i=V(e.totalStock);break;default:return}let s=document.createElement("div");s.innerHTML=i;let d=s.firstElementChild;o.insertBefore(d,o.firstChild);let p=d.querySelector('[id$="-ask"]');if(p&&this.onAskClick){let c=this.onAskClick,l=document.querySelector("h1")?.textContent?.trim()||"";p.addEventListener("click",()=>{c(e.sku,l,t==="overlimit"?n:void 0,t==="price-zero")})}}destroy(){this.removeAll()}removeAll(){for(let t of et)document.getElementById(t)?.remove()}};var x=class{constructor(){this.styleEl=null;this.observer=null}apply(t,e,n){let o=document.querySelector(".product-availability dd, .product-availability span, .product-availability .property__value");if(!o)return;let i="",s="";switch(t){case"available":i="dost\u0119pny",s="#16a34a";break;case"overlimit":i="dost\u0119pny",s="#16a34a";break;case"out-of-stock":i="zapytaj o dost\u0119pno\u015B\u0107",s="#b45309";break;case"price-zero":i="zapytaj o cen\u0119 i dost\u0119pno\u015B\u0107",s="#b45309";break}if(!i)return;let d="techtor-avail-css";this.styleEl||(this.styleEl=document.createElement("style"),this.styleEl.id=d,document.head.appendChild(this.styleEl)),this.styleEl.textContent=`
    ...
    <div style="background:#fff;max-width:780px;width:100%;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,0.15);max-height:90vh;overflow-y:auto;padding:28px;position:relative;">
      ...
      <p style="margin:0 0 20px;font-size:13px;color:#6b7280;">Produkt: <strong>${h(t)}</strong> (${h(r)})</p>
  ...
  `,document.body.appendChild(o),document.getElementById("techtor-ask-close").onclick=()=>o.remove();let d=o.querySelector('input[name="nip"]');d&&d.addEventListener("input",async()=>{let c=d.value.replace(/\D/g,"");if(c.length===10)try{let f=await(await fetch(`${rt}?nip=${c}`)).json();if(f?.result?.subject){let u=f.result.subject,M=o.querySelector("form"),_=(q,b)=>{let T=M.querySelector(`[name="${q}"]`);T&&!T.value&&(T.value=b)};_("company",u.name||""),_("street",u.workingAddress?.split(",")[0]||"");let I=(u.workingAddress||"").split(",");if(I.length>1){let b=I[I.length-1].trim().match(/^(\d{2}-\d{3})\s+(.+)/);b&&(_("zip",b[1]),_("city",b[2]))}a("NIP auto-fill:",u.name)}}catch{}});let p=document.getElementById("techtor-ask-form");p.onsubmit=async c=>{c.preventDefault();let l=new FormData(p),f=(l.get("name")||"").trim(),u=(l.get("email")||"").trim();if(!f||!u||!u.includes("@")){alert("Wype\u0142nij wymagane pola (imi\u0119, email)");return}try{(await fetch(nt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:f,email:u,_hp:l.get("_hp")||"",phone:l.get("phone")||"",nip:l.get("nip")||"",company:l.get("company")||"",street:l.get("street")||"",zip:l.get("zip")||"",city:l.get("city")||"",message:l.get("message")||"",quantity:l.get("quantity")||"1",sku:r,product:t,url:location.href})})).ok?(p.style.display="none",document.getElementById("techtor-ask-success").style.display="block",setTimeout(()=>o.remove(),3e3)):alert("Wyst\u0105pi\u0142 b\u0142\u0105d. Spr\xF3buj ponownie.")}catch{alert("B\u0142\u0105d po\u0142\u0105czenia. Spr\xF3buj ponownie.")}}}var S=class{constructor(t){this.modules=[];this.currentState="loading";this.currentQty=1;this.currentSku="";this.adapter=t}start(){this.injectStyles();let t=new v;t.setAskHandler(H),this.modules=[new g,t,new x,new k,new w],this.adapter.onProductChange(n=>{a("SKU changed:",n),this.currentSku=n,this.update()}),this.adapter.onQuantityChange(n=>{a("Qty changed:",n),this.currentQty=n,this.update()}),this.adapter.onNavigation(()=>{a("Navigation detected \u2014 restart"),this.restart()});let e=m();if(e)this.currentSku=e,this.update();else{let n=setInterval(()=>{let o=m();o&&(clearInterval(n),this.currentSku=o,this.update())},200);setTimeout(()=>clearInterval(n),1e4)}window._tRerun=()=>this.restart(),a("Widget started, mode:",window.__techtorWidget?.mode)}update(){if(!this.currentSku)return;let t=O(this.currentSku),e=D(t,this.currentQty);R(this.currentState,e,t,this.currentQty),this.currentState=e,window.__techtorWidget&&(window.__techtorWidget.state=e);for(let n of this.modules)n.apply(e,t,this.currentQty)}restart(){a("Widget restart");for(let t of this.modules)t.destroy();$(),this.currentState="loading",this.currentQty=1,this.currentSku="",C().then(()=>{let t=new v;t.setAskHandler(H),this.modules=[new g,t,new x,new k,new w];let e=m();e&&(this.currentSku=e,this.update())})}injectStyles(){if(!document.getElementById("techtor-widget-css")){let t=document.createElement("style");t.id="techtor-widget-css",t.textContent=N+`
`+Z,document.head.appendChild(t)}}};async function Y(){if(!window.__techtorWidget?.initialized)if(window.__techtorWidget={version:3,runId:0,initialized:!1,mode:"loading"},P(),a("Widget v3 bootstrap start"),await C(),typeof window.useStorefront=="function")window.useStorefront(r=>{if(window.__techtorWidget.initialized)return;window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="eventbus",a("Tryb: Event Bus (useStorefront)");let t=new E(r);new S(t).start()}),setTimeout(()=>{if(!window.__techtorWidget.initialized){A("useStorefront timeout \u2014 fallback na DOM adapter"),window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="dom-fallback";let r=new y;new S(r).start()}},5e3);else{A("Brak useStorefront \u2014 DOM fallback"),window.__techtorWidget.initialized=!0,window.__techtorWidget.mode="dom-fallback";let r=new y;new S(r).start()}}Y().catch(r=>console.error("[TECHTOR] Widget bootstrap error:",r));})();
